...
// --------------------------------------------------------------------
// below are added by j.zhou, Sep 18, 2017
...
LIBLTE_ERROR_ENUM liblte_rrc_parse_dcch_msg_header(LIBLTE_BYTE_MSG_STRUCT *msg, uint8* ext, uint8* msg_type) {
    LIBLTE_ERROR_ENUM  err     = LIBLTE_ERROR_INVALID_INPUTS; 
    uint8             *msg_ptr = msg->msg;

    if(msg != NULL && ext != NULL && msg_type != NULL)
    {
        // Extension indicator
        *ext = (*msg_ptr) >> 7;
        *msg_type = ((*msg_ptr) >> 3) & 0x0f;
...
LIBLTE_ERROR_ENUM liblte_rrc_parse_ul_ccch_msg_header(LIBLTE_BYTE_MSG_STRUCT *msg, uint8* ext, uint8* msg_type) {
    LIBLTE_ERROR_ENUM  err     = LIBLTE_ERROR_INVALID_INPUTS; 
    uint8             *msg_ptr = msg->msg;
...
LIBLTE_ERROR_ENUM liblte_rrc_parse_dl_ccch_msg_header(LIBLTE_BYTE_MSG_STRUCT *msg, uint8* ext, uint8* msg_type) {
    LIBLTE_ERROR_ENUM  err     = LIBLTE_ERROR_INVALID_INPUTS; 
    uint8             *msg_ptr = msg->msg;
