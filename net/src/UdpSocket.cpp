    ...
    // sync mode
        ...
        if (result != SKT_ERR) {
            ...
            if (numOfByteRecved < 0) {
                LOG4CPLUS_ERROR(_NET_LOOGER_NAME_, "no data ready on the socket receive buffer");
                numOfByteRecved = 0;
            }
...
int UdpSocket::send(const char* theBuffer, int numOfBytesToSend, InetAddressPort& theRemoteAddrPort) {
    ...
    // sync mode
        ...
        if (result == SKT_ERR) {
            LOG4CPLUS_ERROR(_NET_LOOGER_NAME_, "error occur on sendto() in sync mode, close the socket.");
            close(); // TODO
...
void UdpSocket::close() {
    LOG4CPLUS_DEBUG(_NET_LOOGER_NAME_, "UdpSocket::close(), fd = " << this->getSocket());  
    if (this->getSocket() < 0) {
        return;
    }
