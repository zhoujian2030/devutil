...
// -------------------------------------
void LoggerInit(const char* pConfigFile)
{
    if (pConfigFile == 0) {
        return;
    }

    int fd = open(pConfigFile, O_RDONLY, 0);
    if (fd == -1) {
        printf("[%s], open file[%s] error\n", __func__, pConfigFile);
        return;
    }
    ...
    int numBytesRead = read(fd, jsonBuffer, 4096);
    if (numBytesRead == -1) {
        printf("[%s], read file error, fd = %d\n", __func__, fd);
        return;
    }

    if (numBytesRead == 4096) {
        printf("[%s], numBytesRead = 4096\n", __func__);
        return;
    }
    ...
    LoggerParseConfig(jsonRoot, &loggerConfig);
    ...
    LoggerInitConfig(&loggerConfig);
}

// -------------------------------------
void LoggerInitConfig(LoggerConfig* pConfig)
{
    if (gLoggerStatus_s.initFlag) {
    ...
    if (pConfig != 0) {
        ...
        if (pConfig->logToSocketFlag) {
            if (gLoggerStatus_s.socket != -1) {
                SocketClose(gLoggerStatus_s.socket);
            }

            gLoggerStatus_s.socket = SocketUdpInit();
            SocketMakeNonBlocking(gLoggerStatus_s.socket);
        ...
        gLoggerStatus_s.initFlag = 1;

        printf("Configure logger complete.\n");
    }   
}

// -------------------------------------
void LoggerGetConfig(LoggerConfig* pConfig)
{
    if (pConfig != 0) {
        memcpy((void*)pConfig, (void*)&gLoggerConfig_s, sizeof(LoggerConfig));
    }
}
        ...
        if (gLoggerConfig_s.logType != SYNC_LOG) {
            ...
            if (gLoggerConfig_s.logType == AYNC_LOG_TYPE_1) {
                ...
                if ((n > MAX_LOG_ARGS_NUM) || (s > 0) || (ld > 0)) {
                    vsnprintf(pLogInfoNode->u.logMsgInfo.logContent, MAX_LOG_CONTENT_LENGTH - 1, fmt, args);
...
static void LoggerOutputLog(char* logBuff, unsigned length)
{
    ...
    if (gLoggerConfig_s.logToSocketFlag) {
        SocketUdpSend(gLoggerStatus_s.socket, logBuff, length, &gLogServerAddress);  
    }
}
